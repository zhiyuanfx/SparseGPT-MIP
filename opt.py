...
@torch.no_grad()
def opt_sequential_mip(model, dataloader, dev):
    print('Starting ...')
    ...
    for i in range(len(layers)):
        ...
        for name in subset:
            ...
            pruner = MIPPruner(subset[name])
            if args.wbits < 16:
                pruner.quantizer = Quantizer()
                pruner.quantizer.configure(args.wbits, perchannel=True, sym=False, mse=False)
            pruners[name] = pruner
        ...
        def add_batch(name):
            ...
            return tmp
        handles = [subset[name].register_forward_hook(add_batch(name)) for name in pruners]
        for j in range(args.nsamples):
            outs[j] = layer(inps[j].unsqueeze(0), attention_mask=attention_mask)[0]
        for h in handles:
            h.remove()
...
if __name__ == '__main__':
    ...
    solver_fn = opt_sequential if args.solver == 'sparsegpt' else opt_sequential_mip
