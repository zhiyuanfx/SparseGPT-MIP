...
def llama_sequential(model, dataloader, dev):
    ...
    quantizers = {}
    rope = LlamaRotaryEmbedding(model.config, device=dev)
    ...
    for i in range(len(layers)):
        ...
        for names in sequential:
            ...
            for j in range(args.nsamples):
                cos, sin = rope(inps[j].unsqueeze(0), position_ids)
                outs[j] = layer(inps[j].unsqueeze(0), attention_mask=attention_mask, position_embeddings=(cos, sin))[0]
            for h in handles:
                h.remove()
...
@torch.no_grad()
def llama_sequential_mip(model, dataloader, dev):
    print("Starting...")
        ...
        for names in sequential:
            ...
            for name in subset:
                if (
                    ...
                    continue
                pruners[name] = MIPPruner(subset[name])
                if args.wbits < 16:
                    pruners[name].quantizer = Quantizer()
                    pruners[name].quantizer.configure(
                        args.wbits, perchannel=True, sym=False, mse=False
                    )

            def add_batch(name):
                def tmp(_, inp, out):
                    pruners[name].add_batch(inp[0].data, out.data)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="True if pruned using dense before now, false otherwise."
    )
    ...
    for dataset in ['wikitext2', 'c4']:
        ...
        _, testloader = get_loaders(
            dataset, seed=args.seed, model=args.model, seqlen=model.seqlen, is_train=True
        )
        llama_eval(model, testloader, DEV, dataset, args.log_wandb)

        print("test:")
        _, testloader = get_loaders(
            dataset, seed=args.seed, model=args.model, seqlen=model.seqlen, is_train=False
        )
        llama_eval(model, testloader, DEV, dataset, args.log_wandb)
