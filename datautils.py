...
def get_wikitext2(nsamples, seed, seqlen, model, tokenizer, is_train):
...
def get_c4(nsamples, seed, seqlen, model, tokenizer, is_train):
    ...
    valenc = None
    if is_train:
      valenc = torch.cat([pair[0] for pair in trainloader], dim=1) 
    else:
